	...
	file_dialog_complete: function(num_selected, num_queued)
	{
		if (num_queued > 0)
		{
			...
			$('upload').removeClass('disabled')
				.set('disabled', '')
				.set('text','Upload ' + Upload.files_queued + ' Files');
		}
	},
...
//		div = $('<div></div>').attr({ 'id': file.id, 'class': 'photo' });
		div = new Element('div', { 'id': file.id, 'class': 'photo' });
		div.adopt(new Element('div', {'class':'name'})).set('html', file.name.substring(0, 10) + '...');
		div.adopt(new Element('div', {'class':'status'}).set('html', 'pending'))
		div.adopt(new Element('div', {'class':'progress'}).adopt(new Element('div')))
		div.injectTop($('images'));
	},
	...
	*/
	...
	{
		$$('#'+file.id + ' div.status')[0].set('html','uploading...');
	},
	...
	*/
	upload_progress: function(file, bytes, total)
	{
		...
		$$('#'+file.id+ ' div.progress div')[0].setStyle('width', percent + '%')
	...
	*/
	...
	{
		tmp = new Element('div', {'html':data});
		
		tmp.replaces($(file.id)).highlight()
	},
